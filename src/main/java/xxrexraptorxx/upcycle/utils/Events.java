...
import xxrexraptorxx.upcycle.main.References;
import xxrexraptorxx.upcycle.main.Upcycle;
...
public class Events {
            ...
            if (!hasShownUp && Minecraft.getInstance().screen == null) {
                ...
                if (modContainer != null) {
                    ...
                    if (versionCheckResult.status() == VersionChecker.Status.OUTDATED || versionCheckResult.status() == VersionChecker.Status.BETA_OUTDATED) {
                        ...
                        Upcycle.LOGGER.error(References.NAME + "'s version checker failed!");
                        hasShownUp = true;
                    }
                }
            }
    ...
    public static void SupporterRewards(PlayerEvent.PlayerLoggedInEvent event) {
        ...
        if (Config.PATREON_REWARDS.get()) {
            try {
                ...
                Upcycle.LOGGER.error("An unexpected error occurred while getting the Supporter URI lists", e);
            }
        }
    }
    ...
    private static boolean SupporterCheck(URI uri, Player player) {
        try {
            ...
            Upcycle.LOGGER.error("Failed to fetch or process supporter list from URI: {}", uri, e);
            return false;
        }
    }
    ...
    private static boolean SupporterCheck(URL url, Player player) {
        try {
            ...
            Upcycle.LOGGER.error("Supporter list URL not found! >>{}", url);

        } catch (Exception e) {
            Upcycle.LOGGER.error("An unexpected error occurred while checking supporter list", e);
        }
